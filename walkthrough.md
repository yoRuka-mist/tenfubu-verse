...
## 2025/12/21 オーディオ・ビジュアルの改善
### 7. オーディオの実装
- **ランダムBGM**: `ouka.mp3` を追加し、起動（リロード）のたびに `Green Misty Mountains.mp3` とランダムで切り替わるようにしました。
- **特定アニメーションSE**:
    - `shot` アニメーション: `shot.mp3`（発射音）と `hibi.mp3`（余韻音）を時間差で再生。
    - `impact` アニメーション: `panch1.mp3`（打撃前半）と `panch2.mp3`（衝撃音）を連続して再生。
- **カードドロー音**: `card.mp3` をドローアニメーションの開始に合わせて再生。

### 8. レイアウトとイラストの調整
- **手札の接地感向上**: `translateY` の数値を調整し、通常時・展開時ともにカードが画面下端にしっかり接地するようにしました。
- **なゆた画像更新**: `nayuta.jpg` から `nayuta.png` へ参照を切り替えました。

## 確認方法
1. **オーディオ**:
    - ページをリロードし、BGMが切り替わることを確認。
    - カードをドローした際、めくる音が聞こえるか確認。
    - `shot` や `impact` のエフェクトを持つカードで攻撃し、効果音が重なって聞こえるか確認。
2. **レイアウト**:
    - 手札のカードが画面の下端に以前より近づいていることを確認。
3. **なゆた**:
    - 「なゆた」のカード画像が新しいPNGファイルに変わっていることを確認。
4. **AIの挙動**:
