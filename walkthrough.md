...
### 場のカード選択時の進化ボタンUI追加
- **対象ファイル**: `game/src/screens/GameScreen.tsx`
- **目的**: 自分の場のカードを選択した際、プレイボタンの代わりに進化・超進化ボタンを横並びで表示
- **仕様**:
  - 進化可能時: 黄色のグラデーション（#faf089 → #d69e2e）
  - 超進化可能時: 紫色のグラデーション（#b794f4 → #805ad5）
  - 不可時: 灰色（#718096）
  - ボタン幅: バトルログの横幅の半分（gap分を考慮）
  - 進化可能ターンに限らず常に両ボタンを表示

## 変更内容詳細

### 1. selectedCard型の拡張
- `source: 'HAND' | 'BOARD' | 'LOG'` フィールドを追加
- 手札選択時は `source: 'HAND'`
- 場のカード選択時は `source: 'BOARD'`
- バトルログから選択時は `source: 'LOG'`

### 2. setSelectedCard呼び出し箇所の修正
- 2431行目: バトルログからの選択 → `source: 'LOG'`
- 5144, 5151行目: 手札カード選択 → `source: 'HAND'`
- 5213行目: 自分の場のカード選択 → `source: 'BOARD'`
- 7103行目: 相手の場のカード選択 → `source: 'BOARD'`

### 3. プレイボタン条件の変更
- 7783行目: `selectedCard.source === 'HAND'` 条件を追加
- 手札カード選択時のみプレイボタンを表示

### 4. 進化ボタンUIの追加（PC版）
- 7805-7869行目: 場のカード選択時に進化・超進化ボタンを表示
- ボタン幅 = (logWidth - buttonGap) / 2
- 既に進化済みカードは両ボタン灰色
- canEvolveUI/canSuperEvolveUIで使用可否を判定

### 5. 進化ボタンUIの追加（モバイル版）
- 8347-8408行目: モバイル版の進化ボタンUI
- PC版と同様のロジックでスケール対応

## 構造の記録（更新）
- `game/src/screens/GameScreen.tsx`
  - 2416-2420行目: selectedCard型定義拡張
  - 7782-7803行目: プレイボタン（HAND専用）
