...
import { ClassType, BoardCard, Player, Card as CardModel } from '../core/types';
...
const BattleLog = ({ logs }: { logs: string[] }) => {
    ...
    return (
        <div style={{
            position: 'absolute', top: '20%', bottom: '20%', left: 340, width: 250,
            background: 'rgba(0,0,0,0.4)', color: 'white', padding: 10,
            overflowY: 'auto', zIndex: 100, fontSize: '0.8rem',
            fontFamily: 'monospace',
            borderRight: '1px solid #444',
            // Let's keep pointerEvents auto for scroll, but make it semi-transparent.
            pointerEvents: 'auto',
            borderRadius: 8
        }} ref={logRef}>
            <strong>繝舌ヨ繝ｫ繝ｭ繧ｰ</strong>
            <div style={{ marginTop: 10 }}>
                {logs.map((log, i) => (
                    <div key={i} style={{ marginBottom: 4, borderBottom: '1px solid #333' }}>{log}</div>
                ))}
            </div>
        </div>
    );
...
export const GameScreen: React.FC<GameScreenProps> = ({ playerClass, opponentType, gameMode, targetRoomId, onLeave }) => {
    ...
    React.useEffect(() => {
        ...
        let newDamages: { id: number, value: number, x: number, y: number }[] = [];
        ...
        if (gameState.activePlayerId === opponentPlayerId) {
            ...
            const runAiTurn = async () => {
                ...
                // 2.5 Evolve Phase (New!)
                {
                    ...
                    const isFirstPlayer = opponentPlayerId === 'p1';
    ...
    return (
        ...
        >
            ...
            {showMenu && (
                <div style={{
                    ...
                    <div style={{ background: '#2d3748', padding: 40, borderRadius: 16, display: 'flex', flexDirection: 'column', gap: 20, minWidth: 300, border: '1px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
                        <h2 style={{ margin: 0, textAlign: 'center', borderBottom: '1px solid #4a5568', paddingBottom: 10, color: '#fff' }}>繝｡繝九Η繝ｼ</h2>

                        {!showSettings ? (
                            <>
                                <button onClick={() => setShowMenu(false)} style={{ padding: 15, background: '#4a5568', border: 'none', color: 'white', borderRadius: 8, cursor: 'pointer', fontSize: '1.2rem' }}>蜀埼幕</button>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                                ...
                                <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: 'white' }}>
                                    BGM 譛牙柑
                                    <input type="checkbox" checked={audioSettings.enabled} onChange={e => setAudioSettings(prev => ({ ...prev, enabled: e.target.checked }))}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setAudioSettings(prev => ({ ...prev, bgm: parseFloat(e.target.value) }))}
                                        style={{ width: '100%' }}
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setAudioSettings(prev => ({ ...prev, se: parseFloat(e.target.value) }))}
                                        style={{ width: '100%' }}
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setAudioSettings(prev => ({ ...prev, voice: parseFloat(e.target.value) }))}
                                        style={{ width: '100%' }}
                                    />
                                </div>
            ...
            <div style={{ width: '20%', minWidth: 250, borderRight: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', padding: 20, display: 'flex', flexDirection: 'column', zIndex: 20 }} onClick={e => e.stopPropagation()}>
                ...
                <h3 style={{ color: '#a0aec0', borderBottom: '1px solid #4a5568', paddingBottom: 10, marginTop: 0 }}>繧ｫ繝ｼ繝画ュ蝣ｱ</h3>
                {selectedCard ? (
                    ...
                    <div style={{ marginTop: 20, color: '#718096', fontStyle: 'italic' }}>繧ｫ繝ｼ繝峨ｒ驕ｸ謚槭＠縺ｦ隧ｳ邏ｰ繧定｡ｨ遉ｺ</div>
                )}
            </div>
                ...
                {isGameStartAnim && (
                    <div style={{
                        ...
                        <div style={{
                            ...
                            GAME START
                            <div style={{ fontSize: '1.5rem', color: 'white', marginTop: 10, letterSpacing: 5 }}>蟇ｾ謌ｦ髢句ｧ・/div>
                        </div>
                    </div>
                )}
                ...
                <div style={{ height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', width: '100%' }}>
                    ...
                    >
                        ...
                        <div style={{
                            ...
                            {opponent.name === 'Opponent' ? '逶ｸ謇・ : opponent.name}
                        </div>
                    </div>
                </div>
        ...
        return () => clearInterval(timer);
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={{ fontSize: '2rem', color: 'white', marginTop: 10 }}>{isVictory ? '蜍晏茜' : '謨怜圏'}</div>
                </div>
