...
- 有効化後にオフにすることも可能（トグル）
- **エクストラPP分を消費してカードをプレイした場合、即座に使用済みとなる**
...
#### PLAY_CARDアクション（追加処理）
エクストラPP消費時の処理:
- エクストラPPがアクティブな状態でカードをプレイ
- 通常PPだけでは足りないコストを支払った場合、エクストラPP分を消費したと判定
- 消費した場合、即座に使用済みフラグを立てる
- `extraPpActive`をfalseにリセット
- PPが負にならないよう0にクランプ
...
- 使用不可時: グレーアウト + 透明度50% + 「済」表示
- アクティブ時: 強調表示（グロー効果、"ON"表示）
- エクストラPP消費後: 即座に非アクティブ化、再使用不可
...
3. **トグル機能**
   ...
   - オフにするとPP-1される
   - **エクストラPP分を消費したら即座に使用済みとなり、トグル不可**

4. **PP消費判定**
   - エクストラPP有効時にカードをプレイ
   - 「通常PP（エクストラPP分を引いた値）< カードコスト」なら消費したと判定
   - 例: 1/9 +1 で 2コストを使用 → 通常PP=0 < 2 → エクストラPP消費
...
- [ ] エクストラPPをトグルでオフにできる
- [ ] エクストラPP分を消費してカードをプレイすると、ボタンが非アクティブになる
- [ ] 消費後、PP表示が正しく0/Xになる（マイナスにならない）
- [ ] 消費後、ボタンに「済」と表示される
